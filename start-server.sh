#!/usr/bin/env bash
# ============================================================================
# rAthena - Gerenciador de Servidor (Login / Char / Map)
# Uso: ./start-server.sh start|stop|restart|status|monitor|autorestart|tail
# ============================================================================
...
monitor() {
  clear
  echo "=== Monitoramento rAthena (login / char / map) ==="
  echo "(Ctrl+C para sair)"
  while true; do
    echo ""
    for svc in login char map; do
...
case "${1:-status}" in
  ...
  tail)
    ...
    tail -n 50 -f "$LOG_DIR/login.log" "$LOG_DIR/char.log" "$LOG_DIR/map.log"
    ;;
